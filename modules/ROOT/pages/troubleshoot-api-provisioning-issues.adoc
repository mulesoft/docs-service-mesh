...
== API Specification File Exceeds Supported 

When provisioning an API with a specification file that is greater than 550 KBs, you might encounter issues with the provision operation or the Service Mesh services.

=== Cause

If you use a large file, the following errors might be displayed when listing pods in the `service-mesh` namespace:
...
+ 
If a status is displayed other than `Running`, the Service Catalog pods might be failing due to lack of resources. You must increment the limits of both pods and then delete the service instance afterwards.
