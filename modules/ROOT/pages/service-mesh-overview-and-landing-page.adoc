...
Anypoint Service Mesh enables accelerated adoption of microservices by providing discoverability, management, and security for non-MuleSoft microservices that are written using different languages and platforms, or are deployed in different environments.
...
With Anypoint Service Mesh, you can connect your applications, data, and devices through APIs in an applications network. You can manage, govern, and secure your application network using Anypoint Platform.
...
Most importantly, Anypoint Service Mesh can perform these tasks across all services, independent of the technology upon which it is built. You can now discover, manage and secure any type of microservice from a central control plane in the Anypoint Platform.
...
The mixer is a highly modular and extensible component, whose key function is to provide a layer of abstraction over the different policy and telemetry backend systems. This enables applications, such as Anypoint Platform to allow the rest of Istio to be agnostic of the backend processing.

The mixer is architected on a general-purpose plug-in model, which provides flexibility in dealing with different infrastructure backend applications. These plug-ins applications, known as adapters, enable the mixer to interface with different infrastructure backend applications that deliver core functionality, such as logging, monitoring, enforcing quotas, performing ACL checks, and so on.
