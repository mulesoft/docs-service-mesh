...
If you created the API using Anypoint Platform, you use the API ID to bind the service. However, if you you used Anypoint Service Mesh to create the API, you must use the Kubernetes API resource name to bind the service.
...
image::bind-api-id.png[Display API ID,75%,75%]
...
. Verify that your microservice port is named according to the requirements described in https://istio.io/docs/setup/additional-setup/requirements/[Istio].
...
image::api-manager-status-for-service-mesh.png[75%, 75%]
...
+
`kubectl get svc -n _namespace_`
. Verify that your microservice port is named based on https://istio.io/docs/setup/additional-setup/requirements/[Istio requirements].
