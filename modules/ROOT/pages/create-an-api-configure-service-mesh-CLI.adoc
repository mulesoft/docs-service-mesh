...
+
You can configure Anypoint Service Mesh to automatically create an HTTP API in Anypoint Platform (no manual logging required). When the REST or SOAP APIs are automatically created with Anypoint Service Mesh, the APIs are still managed internally as an HTTP asset without any definition. 
+
...
** Using <<create-api-ASM-CLI, user credentials>> (username and password), if your users are managed by Anypoint Platform
** Using <<create-api-ASM-CLI-ConnAps, Connected Apps credentials>>, if your users are managed by an external identity provider (IdP) 
...
Enabling the Anypoint Service Mesh autocreate feature automatically creates APIs for your users if they are properly authenticated. How you authenticate these users depends on whether they are managed by Anypoint Platform or by an external IdP.

[[create-api-ASM-CLI]]
=== Autocreate APIs by Using Your User Credentials

If your users are managed by Anypoint Platform, use their username and password credentials for authentication. Before you configure the Anypoint Service Mesh autocreate feature, ensure that you obtain the:
...
| `assetId` | The ID (alphanumeric characters without spaces) of the assets that are a part of an organization | Required | Not applicable
...
| `instanceLabel` | The ID (alphanumeric characters without spaces) of the API instances of an Asset within a given environment | Required | Not applicable
...
=== Autocreate APIs for Users Managed by an IdP
