...
| `groupId` | The ID of the group of assets on Exchange, for example, the organization ID (available from the API Manager landing page or from Access Management) | Optional | Organization ID of the organization that contains the environment. 
| `assetId` | The ID (supports alphanumeric characters without spaces) of the assets that are a part of an organization | Required | Not applicable
| `version` | The version, with the semantic versioning format for major, minor, and patch releases, of the asset on Exchange | Required | Not applicable
| `instanceLabel` | The ID (supports alphanumeric characters without spaces) of the API instances of an asset within a given environment | Required | Not applicable
| `classifier` | The type of API asset to create: `http`, `oas`, or `raml`  | Optional | `http`
| `mainFile` | The name of the main file in the API specification file: YAML or JSON files must match the filename with the specification file and ZIP files must match the filename with the primary file | Specifiy this option for all classifers, exluding `http`. | Not applicable
