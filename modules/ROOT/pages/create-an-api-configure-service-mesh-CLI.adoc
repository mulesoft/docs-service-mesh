...
** Using <<create-api-ASM-CLI, user credentials>> (username and password). This option only supports users managed by Anypoint Platform.
** Using <<create-api-ASM-CLI-ConnAps, Connected Apps credentials>> regardless of the identity provider used for your users, Anypoint Platform or an external one. We strongly recommend you to use this option over user credentials.
