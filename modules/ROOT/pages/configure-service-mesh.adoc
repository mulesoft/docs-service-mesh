...
Services protected by Anypoint Service Mesh return extra headers. These headers are added by Envoy and the gRPC adapter.


You can remove these headers to avoid returning them to the originating client.

To remove the headers:

. Log in to your Ingress server.
. Search and remove the following headers added by Envoy:
* x-envoy-upstream-service-time
* x-envoy-decorator-operation
. Search and remove the following headers added by the gRPC adapter:
* x-mule-report
