...
When you create the binding, Service Mesh creates a three-way association of the service in Kubernetes with the API that you created in Anypoint Platform to represent the service, and the adapter that you created for the service. The adapter uses the unique API ID of the API to fetch policies and apply them to your services that are running in the cluster.

After you create the binding, Service Mesh takes few seconds to complete the binding procedure and updates the status.

//[PA: Why is the connection with the adapter needed here?]
