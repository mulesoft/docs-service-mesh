...
Istio requires every link:https://istio.io/docs/ops/deployment/requirements/[port to be named]. To resolve this error, re-create the service with a named port:
...
When you create the binding, you must provide a service name. If the service name provided does not match the actual service name, policies will not be enforced.
...
.Delete the existing bindings by executing either:
+
...
+
. Re-create them to point to the actual services:
