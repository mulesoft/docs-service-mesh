...
//Tomas is this the actual error that is generated?
...
The Isito platform requires every xref:https://istio.io/docs/ops/deployment/requirements/[port to be named]. To resolve this error, re-create the service with a named port:
...
The applications managed by Anypoint Service Mesh generate a 500 error code with the following error description: `INTERNAL:grpcmule-nto-payment.handler.service-mesh`.`INTERNAL:grpcmule-nto-payment.handler.service-mesh`
