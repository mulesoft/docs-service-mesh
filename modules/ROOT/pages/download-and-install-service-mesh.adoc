...
When you invoke the installer, Istio, Service Catalog, Anypoint Service Mesh Broker, and ClusterIP Service components are installed in your Kubernetes cluster. The installer performs several configuration tasks and verifies that: 

- A supported Kubernetes versions is installed in the cluster.
- Istio, Service Catalog, and Anypoint Service Mesh are not pre-installed.
- Appropriate Anypoint Service Mesh licences and entitlements are applied.
