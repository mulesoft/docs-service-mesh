...
=== Whitelist Headers for Client Authorization-Based Policies

Anypoint Service Mesh is configured with minimal approved (whitelisted) headers for client-based policies:
...
Before using a custom header, you must whitelist it. Because header whitelisting is performed at the cluster level, you do not need to whitelist headers for every managed API.
...
To list existing whitelisted headers, run the following command:
