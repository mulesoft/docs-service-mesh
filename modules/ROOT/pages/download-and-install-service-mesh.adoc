...
=== Configure headers to use in client authorization based policies

Service Mesh is configured with a reduced set of whitelisted headers for client based policies (case is insensitive). Those headers are:
...
If you want to use a custom header you need to whitelist it. Whitelisting a header is an operation done at cluster level. This means that you don't need to do it for every managed API.

In order to whitelist a header, you need to execute:

`./service-mesh management header-whitelist --name=<header name>`

So for example, if you want to allow the use of header `x-client-secret`, you need to execute:

`./service-mesh management header-whitelist add --name=x-client-secret`

In order to list the whitelisted headers, execute:

`./service-mesh management header-whitelist list`
