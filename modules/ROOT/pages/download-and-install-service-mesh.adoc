...
Service Mesh is configured with a reduced set of whitelisted headers for client based policies. Those headers are:
...
+
`./service-mesh management whitelist <header name>`
+

So for example, if you want to allow the use of header `x-client-secret`, you need to execute:

+
`./service-mesh management header-whitelist add x-client-secret`
+

In order to list the whitelisted, execute:
