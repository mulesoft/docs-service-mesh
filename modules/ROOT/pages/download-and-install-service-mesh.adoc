...
Before you install Service Mesh, you must first download the product from the installer download URL. 
...
When the installer is invoked, Istio and Service Mesh components are installed. The installer performs several steps as part of the installation, such as checks for dependencies and various configurations. If you failed to complete any of the prerequisites listed in the Review Prerequisites section, your installation might fail.

As part of installation, the following required components are deployed on a Kubernetes cluster:
