...
If you created the API using Anypoint Platform, you use the API ID to bind the service. However, if you used the CRD to create the API, you must use the API Kubernetes resource name to bind the service.
...
. Verify that your microservice port is named following Istio standards: xref:https://istio.io/docs/setup/additional-setup/requirements/[].
...
spec:
  instanceRef:
    name: <Adapter Kubernetes resource name> # example: bookinfo-service-mesh-adapter
  parameters:
...
spec:
  instanceRef:
    name: <API Kubernetes resource name> # example: orders-api
  parameters:
    adapter: <Adapter Kubernetes resource name> # example: bookinfo-service-mesh-adapter
    service: <Kubernetes resource service name> # example: orders-service
----
+
