...
+
`$  asmctl adapter list`
. If the adapter status is Failed, review the binding logs:
+
`$ asmctl api binding logs --namespace=_replace_with_namespace_ --name=_replace_with_binding_name_`
+
...
+ 
`$ asmctl api binding delete --namespace=_replace_with_namespace_ --name=_replace_with_binding_name_`
