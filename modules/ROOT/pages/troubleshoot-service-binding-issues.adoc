...
Before you create a binding, the referenced API or adapter must be ready. Otherwise, you receive the 'ErrorInstanceNotReady' error.

=== Causes

This error can occur if you are attempting to create:
 
* An API binding and the API is not ready. 
+
This can occur because the API provisioning failed or has not yet completed. 
* An adapter binding and the adapter is not ready. 
+
This can occur because the adapter provisioning failed or has not yet completed.
...
* `$ asmctl adapter binding list`
+
The status of the binding displays as `ErrorInstanceNotReady`. 

Check the status of the referenced instance using any of the following commands, depending on whether you are using an API or adapter binding:
...
If the adapter or API provisioning has not yet completed (`Provisioning` status), wait until the operation completes. If the status is `Failed`, review the corresponding troubleshooting section:
...
To delete the API or adapter, delete the binding first. After the API or the adapter is ready, you can recreate the binding.
