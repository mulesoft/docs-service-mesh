...
Anypoint Service Mesh is an independent architecture layer, which is encapsulated in a Kubernetes or Red Hat OpenShit cluster. In the Anypoint Service Mesh architecture, instead of the services directly communicating with one another, a sidecar proxy is used to perform the same job.  
