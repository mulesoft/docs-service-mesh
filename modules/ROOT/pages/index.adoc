...
With Anypoint Service Mesh 1.1, the mixer component is removed and its functionality is performed by the Anypoint Service Mesh Envoy filter. With the Envoy filter, requests are directly sent to the adapter, thereby minimizing additional processing.
...
The MuleSoft adapter connects components to Anypoint Platform. The adapter and broker enable sharing of metadata about all the services that are managed by the mesh. This allows for discovery of your existing microservices from the Kubernetes Cluster as APIs into Anypoint Platform. 
...
. Envoy captures the request and redirects it to the Mule adapter using the Envoy filter.
. The adapter then sends the request to Anypoint Platform.
to perform policy checks and verifications.
...
. Periodically, the adapter sends back the API analytics information to Anypoint Platform.
