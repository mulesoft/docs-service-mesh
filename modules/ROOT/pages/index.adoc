...
Because the mixer component is deprecated with Istio v1.5, the authorization mechanism aspect of the mixer  is now implemented by leveraging Anypoint Service Mesh Envoy filters. With the Envoy filter, requests are directly sent to the adapter, thereby minimizing additional processing.
