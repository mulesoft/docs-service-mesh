...
+
Anypoint Service Mesh enables you to automatically create an API in Anypoint Platform using username and password, with no REST or SOAP definitions. You do not have to manually log in to Anypoint Platform. Use this option if your users are managed by Anypoint Platform. 
+
When creating APIs that require specification files, such as REST and SOAP, ensure that the specification file is base64 encoded and embedded into the CRD. Specification files used in this provisioning approach must not exceed 550 KBs in size.
* <<create-api-CRD-ConnAps, Create an API using Anypoint Service Mesh Autocreate with Connected Apps>>
+
...
+
Use this option if your users are managed by Anypoint Platform or an external identity provider (IdP).
...
To create an API in Anypoint Platform using Anypoint Service Mesh with Connected Apps:
...
Anypoint Service Mesh supports the use of Kubernetes secrets to store and manage sensitive information, such as your Connected App credentials.
