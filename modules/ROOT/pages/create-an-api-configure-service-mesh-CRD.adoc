...
** Using <<create_api-CRD, user credentials>> (username and password), if your users are managed by Anypoint Platform  
** Using <<create-api-CRD-ConnAps, Connected Apps credentials>>, if your users are managed by Anypoint Platform or an external identity provider (IdP) 
...
If your users are managed by an external IdP, use the Connected App credentials for authentication. 

Before you configure the autocreate feature, ensure that:
