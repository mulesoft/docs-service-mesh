...
. To store user credentials, create the following Kubernetes secret in a file, for example `user-secret.yaml`:
+
...
+
. Referencing the secret in your API CRD, instead of the parameters user and password, as illustrated in the following example:
+
[source,text,linenums]
----
