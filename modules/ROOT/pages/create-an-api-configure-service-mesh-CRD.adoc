...
+
When creating file-required APIs (REST, SOAP), the specification file content needs to be base64 encoded and embedded into the CRD. Due to Kubernetes limitations, specification files used in this provisioning approach should not exceed 550KB in size.
...
spec:
  ...
  parameters:
    ...
    specPath: _spec path_ # Optional - example: /path/to/api.yaml
    tags:
      apiSpec:
