...
. First of all we need to create a secret to hold the sensitive information, it can be created by Devops/Admin, and save it with a '.yaml' file extension, for example `orders-api-config.yaml`:
+
[source,text,linenums]
----
apiVersion: v1
kind: Secret
metadata:
  name: <secret name> # example: orders-api-config
  namespace: <namespace name> # example: default
type: Opaque
stringData:
  config: |
    {
      "user": "<replace-with-anypoint-username>",
      "password": "<replace-with-anypoint-clientSecret>"
    }
----
...
spec:
  ...
  parameters:
    ...
    tags:
      ...
      apiInstance:
      - <tag in API Manager instance> # example: orders
  parametersFrom: # we will refer to the secret holding the config from here
    - secretKeyRef:
        name: <secret name> # example: orders-api-config
        key: config
----
