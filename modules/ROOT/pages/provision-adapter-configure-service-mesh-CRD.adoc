...
After you provision the adapter, you must set the `istio-injection=enabled` label on the namespace, you can run the following command to add it `kubectl label ns <namespace> istio-injection=enabled`, and redeploy all your existing applications in that namespace to ensure that the Envoy sidecar is injected within each pod in the Kubernetes Cluster.
