...
. First of all we need to create a secret to hold the sensitive information, it can be created by Devops/Admin, and save it with a '.yaml' file extension, for example `bookinfo-service-mesh-adapter-config.yaml`:
+
[source,text,linenums]
----
apiVersion: v1
