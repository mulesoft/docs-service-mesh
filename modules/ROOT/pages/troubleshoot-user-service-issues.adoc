...
When you attempt to run a service, you might encounter issues with the deployment, such as problems with a deployment port, or load balancer issues. This section provides possible causes for your deployment and service issues and describes how to troubleshoot these errors:
...
. Perform the following action based on the status from the diagnosis step:
** `ErrImagePull` or `ImagePullBackOff`: The image cannot be pulled. Ensure that the image value in the deployment spec or pod references a valid repository or image name.
** `Pending`: Ensure that the pod has the required resources. Either edit the deployment to request lower CPU and memory, or scale the cluster.
** `CrashLoopBackOff`: Describe the pod to get more information about the failure:
+
`$ kubectl -n <namespace> describe pod <pod-name>`.
. In the events section of the output generated from the previous step, check if any logs are available describing the status of the deployment: live or ready.
...
Exposing the service provides an externally-accessible IP address that sends traffic to the correct port on your cluster nodes provided your cluster runs in a supported environment and is configured with the correct cloud load balancer provider package.

If you failed to expose the service, any request to the IP address of the service goes into an unresponsive state.

=== Cause

This issue can occur if you skipped exposing the service as load balancer.


=== Diagnose

To diagnose the issue with the service, run the following command from the Command prompt: 
...
To resolve this issue, verify the service specifications. You must configure the service specification as a load balancer to be render it publically available. 
