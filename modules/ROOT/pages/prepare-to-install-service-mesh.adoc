...
[%header%autowidth.spread,cols="4+a"]
|===
|Plane |Host |Port| Explanation
|*US*|*anypoint.mulesoft.com* | 443| Anypoint
|*US*|*analytics-ingest.anypoint.mulesoft.com* |  443| Required to send analytics to the control plane.
|*US*|**.dkr.ecr.us-east-1.amazonaws.com* |  443| Explanation| Required to download the docker images that make up the Anypoint Service Mesh product
|*US*|*exchange-files.anypoint.mulesoft.com* |  443| Required to download the docker images that make up the Anypoint Service Mesh product
|*US*|*exchange2-asset-manager-kprod.s3.amazonaws.com* |  443| Required to download policies
|*EU*|*eu1.anypoint.mulesoft.com* | 443| Anypoint
|*EU*|*analytics-ingest.eu1.anypoint.mulesoft.com* |  443| Required to send analytics to the control plane.
|*EU*|**.dkr.ecr.eu-central-1.amazonaws.com* |  443| Required to download the docker images that make up the Anypoint Service Mesh product
...
|*ALL*|*get.helm.sh* | 443| Required to retrieve and install Helm on the Kubernetes clusters.
|*ALL*|*storage.googleapis.com* | 443| Required to retrieve and install kubectl on the Kubernetes clusters.
|===
