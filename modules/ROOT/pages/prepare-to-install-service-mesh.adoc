...
* <<install_Istio,Istio>> (currently supported versions include 1.7x or 1.8x)
...
To install and configure Istio for Anypoint Service Mesh, run the command from your terminal based on the container platform that you have installed in your environment:
...
+
`$ istioctl install --set components.cni.enabled=true --set addonComponents.prometheus.enabled=false --set components.cni.namespace=kube-system --set values.cni.cniBinDir=/opt/multus/bin --set values.cni.cniConfDir=/etc/cni/multus/net.d --set values.cni.chained=false --set values.cni.cniConfFileName="istio-cni.conf" --set values.sidecarInjectorWebhook.injectedAnnotations."k8s\.v1\.cni\.cncf\.io/networks"=istio-cni`
----
