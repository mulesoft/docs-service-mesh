...
Before you download and install Anypoint Service Mesh, make sure that you review and fulfill the software, licensing, and permissions and roles requirements describes in this section. 

== About the Audience

Anypoint Service Mesh is a enterprise microservices management application. Microservices are deployed in containerized environments to achieve best business results. Anypoint Service Mesh is developed using Istio and Kubernetes. Therefore, you must have basic working knowledge of Istio and Kubernetes to be able to install and use (derive benefit from??) Anypoint Service Mesh.
...
+
*Tip:* To verify the installation, use the command: 
+
`kubectl api-versions | grep admissionregistration.k8s.io/v1beta1`.
* Configure kube-apiserver process with the admission-control flag to “MutatingAdmissionWebhook”
+
For more information about how to turn on *turn on what--Tomas?*, see the Kubernetes documentation.
...
* API Manager Environment Administrator or Organization Administrator permission for your organization, to access the client ID and client secret credentials.
