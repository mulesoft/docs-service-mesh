...
Before you can use Anypoint Service Mesh, you must have the following roles, and credentials:
 
* Organization Administrators role to access the client ID and client secret credentials
* Client ID and client secret credentials to provision the adapter:
+
*Tip:* To access this information in API Manager, go to *API Manager > Environment Information*. 
...
* Anypoint Service Mesh does not support the usage of users that are stored on an external identity provider for the creation or discovery of APIs. For organizations that have configured an identity provider, you need to create a userin Mulesoft's platform. To be able to fully leverage the product's capabilities, that user will need to have _Exchange Contributors_ and _Manage APIs Configuration_ roles.
