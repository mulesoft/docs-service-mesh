...
* <<install_Istio,Istio>> (currently supported versions include 1.6x or 1.7x)
...
In your network, you might need to add to the allow list of hostnames and ports of external resources to enable Anypoint Service Mesh to communicate with the MuleSoft-managed online Anypoint Platform APIs and services.
...
Although Anypoint Service Mesh supports Istio versions `1.6.x` and Istio `1.7.x`, only Istio versions 1.6.0 through 1.6.8, and 1.7.0 are currently tested to work with Aynpoint Service Mesh. You can install any one of these Istio versions in your environment to work with Anypoint Service Mesh.
