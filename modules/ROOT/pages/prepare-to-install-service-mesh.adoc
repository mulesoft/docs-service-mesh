...
* <<Istio v1.4.x or 1.5.x,install_Istio>>
...
* Client ID and client secret credentials (from Access Management), to provision the adapter: 
...
. Download and install Istio using the https://istio.io/docs/setup/[Istio Documentation].
...
components:
  ...
  telemetry:
    enabled: true
----
The following example illustrates the full manifest: 
+
[source,text,linenums]
----
