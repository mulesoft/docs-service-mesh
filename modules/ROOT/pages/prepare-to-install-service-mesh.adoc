...
* Istio 1.4.x installed 
. Follow the official https://istio.io/docs/setup[Istio installation instructions] to install Istio in your cluster.
. Be sure that policy control is explicitly enabled. See Enabling https://istio.io/docs/tasks/policy-enforcement/enabling-policy/[Policy Enforcement] for more details. You need to set these two flags
--set values.global.disablePolicyChecks=false --set pilot.policy.enabled=true
. Be sure to ennable API Analytics by setting this flag disableMixerHttpReports value false. In order to do it, run: 
`kubectl -n istio-system get cm istio -o yaml | sed -e 's/disableMixerHttpReports: false/disableMixerHttpReports: false/g' | kubectl replace -f` 
