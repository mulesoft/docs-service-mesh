...
* If you have configured an external identity provider for your organization, create a user in Anypoint Platform with _Exchange Contributors_ and _Manage APIs Configuration_ roles. Anypoint Service Mesh does not support users residing on an external identity provider for tasks such as creating and discovering APIs.
